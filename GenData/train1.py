...
# from AlexNet import EarlyStopping
from VGG import EarlyStopping
...
def make_experiment(net, args, trainloader, testloader, device):
    ...
    for epoch in range(args.num_epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            # save by minibatch
            if args.save_by_minibatch and i % args.save_freq == 0 and num_saved_counter < args.max_saved:
                num_saved_counter += 1
        ...
        print("accuracy: ", epoch_acc, " loss: ", running_loss)

        if not args.save_by_minibatch and epoch % args.save_freq == 0 and num_saved_counter < args.max_saved:
            num_saved_counter += 1
            file_name = str(exp_name) + "/checkout_{}_{:03d}.pth".format(epoch, i)  # note that we save checkpoint here
            torch.save(net.state_dict(), file_name)
            if i == 0:
                print("File name: {}".format(file_name))
        ...
        # burnin period of 10, not used in prior experiments
        if args.patience != 0 and es.step(torch.tensor(epoch_acc)) and epoch > 10:
            early_stop_num = epoch
            break

    save_configs(args, exp_name, epoch_acc, early_stop_num)
    if args.patience != 0:
        print("hi early_stop at ", early_stop_num)
