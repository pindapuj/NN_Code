...
class Net10(nn.Module):
    def __init__(self, num_classes=10, init_weights=True, p=0.5):
...
# AlexNet with dropout
class Net(nn.Module):
    def __init__(self, num_classes=100, init_weights=True, p=0.5):
        # trained on 2 GPUs, change conv size if trained on CPU
        # trained on CIFAR10 so changed some sizes
        super(Net, self).__init__()
        self.p = p
...
def get_imagenet_data(augment_data=False, batch_size=128):
    train_path = "/cluster/home/it_stu187/iris/data/tiny-imagenet-200/train"
    test_path = "/cluster/home/it_stu187/iris/data/tiny-imagenet-200/test"
    if augment_data:
        transform_train = transforms.Compose(
            [
                transforms.RandomCrop(32, padding=4),
